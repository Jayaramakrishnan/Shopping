...
public class UserController extends BaseController
{
	...
	public Result createUser(String uniqueId)
	{
		...
		try
		{
			...
			if (userDto.getUserName() != RestUrlAttribute.EMPTY_QUOTES && userDto.getUserName() != null)
			{
				...
				{
					CrackersLogger.info(logger, "Inside Email Iterator");
					EmailDto emailDto = iterator.next();
					if (emailDto.getIsPrimary().intValue() == 1)
					{
						CrackersLogger.info(logger, "Email IsPrimary is 1");
						Long userId = userService.validateNewUserMailId(emailDto.getEmail());
						List<Password> expireLinks = userService.expireLinks(userId);
						CrackersLogger.info(logger, "expireLinks List" + expireLinks);
						String keyString = generateSecretKey();
						String encryptedText = CryptoBinderUtil.getEncryptId(emailDto.getEmail());
						CrackersLogger.info(logger, "Encrypted Id" + encryptedText);
						CrackersLogger.info(logger, "Decrypted Id" + CryptoBinderUtil.getDecryptedString(encryptedText));
						...
						CrackersLogger.info(logger, "baseUrl.substring(0, baseUrl.length());" + baseUrl);
						String createPasswordUrl = baseUrl + com.crackers.controllers.web.routes.PasswordController.createPassword(password.getId(), password.getEncryptText()).url();
						CrackersLogger.info(logger, "createPasswordUrl" + createPasswordUrl);
	...
	public Result validateNewPassword()
	{
		...
		try
		{
			CrackersLogger.info(logger, "Inside validateNewPassword");
			...
			if (password != null)
			{
				...
				{
					CrackersLogger.info(logger, "Returning ok if emails are equal");
					return ok(Json.toJson(RestUrlAttribute.EMPTY_QUOTES));
				}
				else
				{
					CrackersLogger.info(logger, "Given mail id doesn't match with User's registered mail Id");
					return badRequest("Given mail id is not registered one.");
				}
			}
			else
			{
				CrackersLogger.info(logger, "Link Got Expired if password object is null");
				return unauthorized("Link Got Expired");
			}
		}
		catch (Exception e)
		{
			CrackersLogger.error(logger, "Error while validating user", e);
			return internalServerError();
		}
	}
	...
	public Result createPassword()
	{
		...
		try
		{
			Long idUser = userService.validateNewUserMailId(userDto.getEmail());
			if (idUser == null)
			{
				CrackersLogger.error(logger, "Mail id is invalid", null);
				return unauthorized();
			}
			CrackersLogger.info(logger, "idUser in createPassword " + idUser);
			boolean passwordsList = userService.verifyPreviousPasswords(idUser, userDto.getNewPassword());
			if (passwordsList)
			{
				CrackersLogger.info(logger, "Password is new and not under previous ones");
				...
				CrackersLogger.info(logger, "password Links List" + password);
			}
			else
			{
		...
		{
			CrackersLogger.error(logger, "Error while creating password for user", e);
			return internalServerError();
		}
	...
	public Result validateForgetUserMailId(String emailId)
	{
		...
		{
			Long idUser = userService.validateNewUserMailId(emailId);
			if (idUser == null)
			{
				CrackersLogger.error(logger, "Your mail id is not registered. Please contact your administrator. Or User is in active", null);
				return ok(Json.toJson(RestUrlAttribute.EMPTY_QUOTES));
			}
			CrackersLogger.info(logger, "emailId" + emailId);
			try
			{
				...
				CrackersLogger.info(logger, "baseUrl.substring(0, baseUrl.length());" + baseUrl);
				String createPasswordUrl = baseUrl + com.crackers.controllers.web.routes.PasswordController.forgotPassword(password.getId(), encryptText, 0).url();
				CrackersLogger.info(logger, "createPasswordUrl" + createPasswordUrl);
				URLEncoder.encode(createPasswordUrl, "UTF-8");
				Map<String, String> map = new HashMap<>();
				...
				{
					CrackersLogger.info(logger, "forgot mail");
				...
				List<UserInfo> userInfos = new ArrayList<>();
				userInfos.add(userInfo);
				CrackersLogger.info(logger, "emailTemplate" + emailTemplate);
				sendEMail.sendMail(emailTemplate, idUser, userRepository.validateById(idUser), userInfos);
				return ok(Json.toJson(RestUrlAttribute.EMPTY_QUOTES));
			}
			catch (Exception e)
			{
				CrackersLogger.error(logger, "Error while validating forget password for user", e);
				return internalServerError();
			}
		}
		CrackersLogger.error(logger, "Your mail id exist in user table,but not in Email table", null);
		return ok(Json.toJson(RestUrlAttribute.EMPTY_QUOTES));
	}
	...
	public Result createForgetPassword()
	{
		...
		try
		{
			...
			{
				boolean passwordsList = userService.verifyPreviousPasswords(idUser, userDto.getNewPassword());
				CrackersLogger.info(logger, "First time passwords reset" + passwordsList);
				if (passwordsList)
				{
					CrackersLogger.info(logger, "IdUser is Valid " + idUser);
	...
	public Result createResetPassword(String uniqueId)
	{
		...
		{
			Long idUser = CacheManager.getIdUserFromCache(uniqueId);
			CrackersLogger.info(logger, "Current User Id" + idUser);
			UserCredential userCredential = userService.getUserCredentialObject(idUser);
			boolean passwordsList = userService.verifyPreviousPasswords(idUser, userDto.getNewPassword());
			if (passwordsList)
			{
				if (userCredential != null)
				{
					...
					CrackersLogger.info(logger, "userCredential.getHashedKey()" + userCredential.getHashedKey() + "oldPassword " + oldPassword);
					if (userCredential.getHashedKey().equals(oldPassword))
					{
						CrackersLogger.info(logger, "Going to Update Password");
						Integer credential = userService.createForgetPassword(idUser, userDto, userCredential.getSaltKey());
						CrackersLogger.info(logger, "After Updating Password");
						userDto.setIdUser(CryptoBinderUtil.getEncryptId(credential.toString()));
					}
					else
					{
						CrackersLogger.info(logger, "Returning BadRequest");
		...
		{
			CrackersLogger.error(logger, "Error while reseting password for user", e);
			return internalServerError();
		}
		return ok(Json.toJson(userDto));
	}
